...
/**
 * 复制FragmentNavigator功能，将Fragment切换的时候，换成hide/show方法
 *
 * @author ZhangXiaoMing 2024-03-29 22:51 周五
 */
@Navigator.Name("fragment")
open class XMFragmentNavigator(
     ...
     * if the pop succeeded so that the newly visible Fragment can be
     * retrieved with [FragmentManager.getPrimaryNavigationFragment].
     *
     * Note that the default implementation pops the Fragment asynchronously,
     * so the newly visible Fragment from the back stack is not instantly
     * available after this call completes.
     */
     ...
     * [FragmentTransaction.setPrimaryNavigationFragment] so that
     * the Fragment associated with the new destination can be
     * retrieved with [FragmentManager.getPrimaryNavigationFragment].
     *
     * Note that the default implementation commits the new Fragment
     * asynchronously, so the new Fragment is not instantly available after
     * this call completes.
     */
    ...
    private fun navigate(
        ...
        if (fragmentManager.fragments.isNotEmpty()) {
            for (fragment in fragmentManager.fragments) {
                if (fragment.javaClass.name.equals(className)) {
                    frag = fragment
                    break
                }
            }
...
//     *
//     * Construct a new fragment destination. This destination is not valid
//     * until you set the Fragment via [setClassName].
//     *
//     * @param fragmentNavigator The [FragmentNavigator] which this destination
//     *     will be associated with. Generally retrieved via a [NavController]'s
...
//         * until you set the Fragment via [setClassName].
//         *
//         * @param navigatorProvider The [NavController] which this destination will
//         *     be associated with.
//         */
...
//         *
//         * @param className The class name of the Fragment to show when you
...
//    }
    ...
    /**
     * Extras that can be passed to FragmentNavigator to enable Fragment
     * specific behavior
     */
    class Extras internal constructor(sharedElements: Map<View, String>) :
        ...
        /**
         * The map of shared elements associated with these Extras. The returned
         * map is an [unmodifiable][Map] copy of the underlying map and should be
        ...
        /**
         * Builder for constructing new [Extras] instances. The resulting instances
         * are immutable.
         */
        class Builder {
            private val _sharedElements = LinkedHashMap<View, String>()
            ...
            /**
             * Adds multiple shared elements for mapping Views in the current Fragment
             * to transitionNames in the Fragment being navigated to.
             *
            ...
            /**
             * Maps the given View in the current Fragment to the given transition name
             * in the Fragment being navigated to.
             *
             * @param sharedElement A View in the current Fragment to match with a View
             *     in the Fragment being navigated to.
             * @param name The transitionName of the View in the Fragment being
             *     navigated to that should be matched to the shared element.
             * @return this [Builder]
